...
#define kCFBundleShortVersionString CFSTR("CFBundleShortVersionString")
...
- (NSString *)version
{
	NSString* key1 = (__bridge NSString *)kCFBundleShortVersionString;
	NSString* key2 = (__bridge NSString *)kCFBundleVersionKey;
	
    NSString *version = [self.bundle objectForInfoDictionaryKey:key1];  // bt_browser added.  prefer long version string
    if (!version || [version isEqualToString:@""])  // bt_browser fall back to CFBundleVersion
        version = [self.bundle objectForInfoDictionaryKey:key2];
    if (!version || [version isEqualToString:@""])
        [NSException raise:@"SUNoVersionException" format:@"This host (%@) has no %@ or ! This attribute is required.", [self bundlePath], key1, key2];
    return version;
}
